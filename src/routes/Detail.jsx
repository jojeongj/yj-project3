...
export default function Detail() {
    ...
    const { isLoading, error, data } = useQuery(paramData[1], () =>
        fetch(`https://gateway.marvel.com:443/v1/public/${paramData[1]}/${id}?apikey=3cde1ed7441b7dcb24742fc915cb1104`).then(res =>
            res.json()
        )
    )
    ...
    return (
        <>
            <Box>
                <VStack w="full" h="650px" backgroundImage={`${data?.data?.results[0].thumbnail.path}.${data?.data?.results[0].thumbnail.extension}`} backgroundRepeat={"no-repeat"} backgroundSize="cover" backgroundPosition="center" alignItems="center" position="relative">
                    {/* 검정색 처리된 빈박스 */}
